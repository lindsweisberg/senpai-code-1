...
prolific.raw = read.csv("~/Experiments/senpai/for_prolific/prolific_export_012218.csv", header = TRUE, stringsAsFactors = FALSE)

qualtrics.raw = fread("~/Experiments/senpai/for_prolific/semantic_content_norming_012218.csv", header = TRUE, stringsAsFactors = FALSE, verbose = TRUE)

stims1 = read.csv("selected_cowen_keltner_video_urls_926_1.csv", header = FALSE, stringsAsFactors = FALSE)
stims2 = read.csv("selected_cowen_keltner_video_urls_926_2.csv", header = FALSE, stringsAsFactors = FALSE)
stims = data.frame(first_half = str_sub(stims1$V1, start = 54L), second_half = str_sub(stims2$V1, start = 54L), stringsAsFactors = FALSE); rm(stims1, stims2)
...
write.csv(qualtrics.count, file = "~/Experiments/senpai/for_prolific/for_approval_012218.csv", row.names = FALSE)
...
famous = qualtrics.long %>%
  ...
  select(q_type, q_gif_num, response) %>%
  distinct() %>%
  separate(q_type, c("q_type", "round"), fill = "right") %>%
  mutate(round = as.numeric(ifelse(is.na(round), "1", round))) %>%
  select(-q_type, -response) %>%
  full_join(filter(select(qualtrics.long, q_type, q_gif_num, response), grepl("famous_cat", q_type) | grepl("famous_names", q_type)), by = "q_gif_num")
...
# Scroll through the dataframe in Viewer; anytime you see the following:
...
# in place of XXXX, find the video's number by indexing the stims dataframe at the correct address
